...
description: >
  ...
  再修正 を繰り返すループ（最大3周）。ボット（CodeRabbit / Copilot / Gemini Code Assist
  など任意）は再レビューまで待って往復し、人間のレビュー指摘も修正して in-thread で
  結果を返信する（人間の再レビューは待たない）。承認プロンプトを挟まず全自動で回す。
...
- レビュアー login（`coderabbitai[bot]` / `copilot-pull-request-reviewer[bot]` など）→
  再レビューを待つ対象ボットをそれに固定する（人間のスレッドの扱いは変わらない）
...
修正の対象は**ボットと人間の両方**だが、ループの回し方が違う:

- **ボット**（再レビュー待ちの対象）: login が指定されなければ、PR の review thread を
  書いたボットを自動検出する — login が `[bot]` で終わる、または既知のレビューボット
  （`coderabbitai`, `copilot-pull-request-reviewer`, `gemini-code-assist` など）。
  複数のボットが指摘を付けていれば全ボットを対象にする
- **人間**: unresolved かつ非 outdated なスレッドがあれば同じ安全ルールで修正するが、
  **再レビューは待たず、収束判定にも数えない**（人間はいつ返信するか分からないため、
...
for round in 1..3:
  1. unresolved かつ非 outdated なスレッドを取得
     → ボットのスレッドが 0、かつ人間の未処理スレッドも無ければ成功終了
  2. 各指摘（ボット + 人間）を検証し、妥当なものだけ修正を適用（下記の安全ルール）
  3. 指摘はあったが 1 件も適用しなかった（全件 defer）→ 打ち切り終了。
     push せず、残指摘を「要人間判断」として報告する
  4. consolidated commit を push。人間のスレッドには in-thread で結果を返信
  5. ボットの再レビュー完了を待つ（ポーリング、最大 15 分）→ 次の周へ。
     ボットのスレッドが元々 0 で人間の指摘だけだった場合は待たずに終了
3周終えても指摘が残る → 打ち切り、残指摘を報告して終了
```

成功の判定は「ボットの unresolved スレッド数 = 0」かつ「人間のスレッドは全件
処理済み（修正して返信済み、または defer して返信済み）」で行う。人間のスレッドは
...
issue report として扱う。人間のコメントも同じ扱いでよい。いずれの場合も
...
## Step 3: push と再レビュー待ち（ボットのみ）

再レビューを待つのは**ボットだけ**。人間のスレッドは Step 4 の in-thread 返信で
処理完了とし、返信を待たない。この周のボットのスレッドが元々 0 だった
（人間の指摘だけを処理した）場合はポーリング自体をスキップして終了処理へ進む。

push の直前に、その時点での対象ボットの最新レビュー時刻を記録しておく。
...
**人間のスレッド**は加えて、修正・defer の結果を各スレッドへの in-thread 返信で
...
- **成功**: ボットの unresolved かつ非 outdated なスレッドが 0、かつ人間のスレッドは
  全件処理済み（修正 or defer を in-thread 返信済み）
- **打ち切り**: 3 周実行しても指摘が残っている / ある周で全件 defer になった
- **停止**: ボットの再レビューが 15 分来ない / push 失敗 / lint・test が revert でも
...
- 対象レビュアー: <login...>（ボット / 人間の別を明記）
- 実行周回: N / 3
- 適用した修正: X 件（commit: <sha>...、うち人間の指摘 H 件は in-thread 返信済み）
...
- 人間のスレッドの扱い（再レビューを待たない・resolve しない・結果は in-thread 返信）
...
- resolved・outdated なスレッド、および引数でボット login が固定されている場合の
  対象外ボットのスレッドには一切触れない

...
- `.agents/skills/` contains Codex skills. `orchestrator` uses custom `planner` / `generator` / `evaluator` agents and built-in `default` agents for review and reporting, `code-review-autofix` handles review round-trips, `start-project` initializes repository-native project documentation, and `codex-setup` installs everything.
